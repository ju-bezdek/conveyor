...
async def add_metadata(result: dict) -> dict:
    ...
    result['pipeline_info'] = {
        'pipeline_id': context.pipeline_id[:8] if context and context.pipeline_id else 'unknown',
        'execution_mode': context.execution_mode if context else 'unknown',
        'current_stage': context.current_stage if context else 'unknown',
        'total_stages': context.stage_count if context else 'unknown'
    }
...
async def main():
    ...
    if context_results:
        ...
        print(f"Stage info: {last_result['pipeline_info']['current_stage']}/{last_result['pipeline_info']['total_stages']}")
